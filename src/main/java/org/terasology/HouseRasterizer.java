...
public class HouseRasterizer implements WorldRasterizer {
    ...
    public void initialize() {
        plank = CoreRegistry.get(BlockManager.class).getBlock("Core:Plank");
    }
    ...
    public void generateChunk(CoreChunk chunk, Region chunkRegion) {
        ...
        for (Entry<BaseVector3i, House> entry : houseFacet.getWorldEntries().entrySet()) {
            ...
            Region3i inside = Region3i.createFromCenterExtents(centerHousePosition, extent - 2);
            ...
            for (Vector3i newBlockPosition : walls) {
                if (chunkRegion.getRegion().encompasses(newBlockPosition)
                    ...
                    chunk.setBlock(ChunkMath.calcBlockPos(newBlockPosition), plank);
                }
            }
        }
    }
